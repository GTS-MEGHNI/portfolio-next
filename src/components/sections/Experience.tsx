          ...
          <div className="relative border-l border-border pl-8 space-y-12">
            {experience.map((job, index) => (
              <div key={`${job.company}-${index}`} className="relative">
                <span
                  className={`absolute -left-[38.5px] top-1.5 w-3 h-3 rounded-full ${index === 0 ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.7)]' : 'border-2 border-accent bg-bg'}`}
                  aria-hidden="true"
                />
                <div className={index === 0 ? 'p-6 bg-surface border border-border hover:border-accent/60 rounded-lg -ml-2 transition-colors duration-200' : 'hover:border-accent/60 transition-colors duration-200'}>
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    <span
                      className={`font-mono text-xs rounded px-2 py-0.5 border ${index === 0 ? 'text-emerald-400 border-emerald-400/40' : 'text-muted border-border'}`}
                    >
                      {index === 0 ? '● running' : '✓ passed'}
                    </span>
                    {job.type && (
